#!/usr/bin/env python3
"""check-and-verify 스킬 번들 — PR/Issue/plan 체크박스 파싱·분류·갱신.
...
서브커맨드:
  classify <body_file>
      ...
      category ∈ {pytest, script, script+env, unity, file_check, manual}
...
def classify_command(span):
    ...
    if re.match(r'python3?\s+(test|tests|scripts)/', body):
        return ('script+env' if has_env else 'script', c)
    if re.search(r'(?:powershell|pwsh)\b.*scripts[/\\]Invoke-Unity\.ps1\b.*\s-Action\s+(?:Compile|GenerateUI|ValidateUI|TestEditMode|TestPlayMode)\b', body, re.IGNORECASE):
        return ('unity', c)
    return (None, None)
...
def classify_item(text):
    ...
    if re.search(r'(존재|생성|저장|출력|파일)', text) and not re.search(r'(0건|없음|없다|미존재)', text):
        for span in CODE_RE.findall(text):
            s = span.strip()
            if ('/' in s or '\\' in s) and ' ' not in s and not s.startswith(('python', 'pytest')):
