#!/bin/bash
# safe_stage.sh — 안전 staging 도우미
#
# 영구 제외 경로(.claude/settings.local.json, .env 등)를 필터링하고,
# 남은 파일 목록을 사용자에게 제시한 뒤 명시적으로 `git add` 한다.
#
# 사용법:
...
#
# 프로젝트별로 추적하지 않을 경로가 생기면 EXCLUDE_PATTERNS 에 추가한다
# (references/excluded_paths.md 참조).
