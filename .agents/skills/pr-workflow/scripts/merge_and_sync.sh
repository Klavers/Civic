#!/bin/bash
# merge_and_sync.sh — PR 머지 + base branch 로컬 동기화 (1 호출)
#
# 머지 후 sync 대상은 PR 의 baseRefName. main 또는 분기 base.
#
# 사용:
...
#   3. gh pr merge --merge  (브랜치 보존 — --delete-branch 미사용)
...
# 브랜치 보존: --delete-branch 미사용 (head branch·worktree 임의 삭제 금지)
"$GH" pr merge "$PR" --merge >/dev/null 2>/dev/null || true
