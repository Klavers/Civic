#!/bin/bash
# detect_base.sh — PR base branch 결정 (+ confidence level)
#
# 우선순위 + 신뢰도:
#   1. 명시 인자 (--base <branch>)            → confidence=HIGH
#   2. 메타 파일: $GIT_DIR/claude-pr-base      → confidence=HIGH
#   3. 환경변수: CLAUDE_PR_BASE_BRANCH         → confidence=HIGH
...
#
# PRIORITY_BRANCHES: 다중 후보 시 우선 채택할 통합 branch 목록.
#   기본 main. develop 등 다른 통합 branch 를 쓰면 추가한다.
#
# 출력:
...
META="$GIT_DIR/claude-pr-base"

# 다중 후보 시 우선 채택할 통합 branch (프로젝트별 조정)
PRIORITY_BRANCHES="main"
...
if [ -n "${CLAUDE_PR_BASE_BRANCH:-}" ]; then
  emit "$CLAUDE_PR_BASE_BRANCH" HIGH env-CLAUDE_PR_BASE_BRANCH
fi
...
priority_matches=""
for pri in $PRIORITY_BRANCHES; do
