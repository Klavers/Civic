...
- **한 차수(round) = 최대 4질문.** `AskUserQuestion` 은 한 번에 4개까지만 받는다(5개 이상 InputValidationError). 의존 관계가 있는 결정을 **한 차수씩 순차 해소**한다 — "한 번에 하나" 는 "한 차수씩" 으로 해석.
- **질문이 5개 이상이면 차수 분할.** 묶어서 한 번에 전달하지 말고 여러 차수로 나누어 연속 수행. 각 차수 응답 후 결과를 문서·계획에 inline 반영 → 다음 차수 진입.
