...
- `stone=석재`, `clothing=의류`, `science=과학`을 유지한다.
...
- 공방/공장 계열 표기는 `의류 공방`, `종이 공방`, `약품 공방`, `유리 공방`, `가구 공방`, `기호품 공장`, `식료품 공장`으로 정규화한다.
...
- 원자료 CSV 3개를 P03 결정에 맞게 정정했다.
  ...
  - 시대별 기술 효과 raw CSV는 `집`, `식료품 공장`, `의류 공방`, 광산 표기 등 P03 정규 명칭으로 교정했다.
...
## §9 #30 건물 중복 정리 결정 (2026-06-27)

- `clothing_workshop`을 의류 생산 건물의 stable ID로 유지하고 표시명을 `의류 공방`으로 통일한다.
- `textile_factory`는 별도 건물로 유지하지 않고 `clothing_workshop`에 통합한다.
- 직조기는 의류 공방에 공구를 추가 투입하면 의류 `+2/s`, 방직기는 석탄을 추가 투입하면 의류 `+2/s`를 생산하는 조건부 효과로 처리한다.
- 전기 방직과 합성섬유도 같은 의류 공방을 대상으로 하여 중복 건물 재도입을 막는다.
- `paper_mill`을 종이 생산 건물의 stable ID로 유지하고 `print_shop`은 제거한다.
- 인쇄술은 종이 공방의 종이 산출을 `+2/s` 증가시키는 효과로 처리한다.
- 원자료 CSV와 런타임 CSV는 위 통합 규칙을 동일하게 사용한다.
- UI 생성기 테스트의 건물 행 수 기대값은 특정 수량을 하드코딩하지 않고 현재 데이터의 건설 가능 건물 수를 사용한다.

### #30 검증 결과

- artifact-tool CSV import·inspect·render: 4개 대상 CSV PASS
- `ValidateData`: PASS (`ValidateData-20260627-220055.log`)
- 최초 `TestEditMode`: 17 passed / 2 failed
  - 원인: 건물 통합으로 건설 가능 건물이 30개에서 28개로 줄었으나 UI 생성기 테스트가 30행을 고정 기대했다.
  - 조치: 현재 데이터 소스의 건설 가능 건물 수를 기대값으로 계산하도록 테스트를 보정했다.
- 최종 `TestEditMode`: 19 passed / 0 failed (`EditMode-20260627-220401.xml`)
- `GenerateUI`: PASS (`GenerateUI-20260627-220536.log`)
- `ValidateUI`: PASS (`ValidateUI-20260627-220601.log`)
- 최종 `TestPlayMode`: 1 passed / 0 failed (`PlayMode-20260627-220621.xml`)
- `dotnet msbuild Civic\Civic.sln /p:RestorePackages=false /p:RestoreIgnoreFailedSources=true`: PASS
