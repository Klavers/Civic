...
## §21 위인 패널 분리·Tooltip 안정화·국가 통합 Modifier 패널 (2026-07-01)

### 21.1 위인 Tooltip 결함과 수정 원칙

- 위인 패널은 매 프레임 다시 바인딩된다. 기존 `CivicModuleActionRow.Bind()`가 직책 선택 영역을 매번 비활성화하고 곧바로 다시 활성화하면서, 같은 행의 `CivicTooltipTrigger.OnDisable()`이 공용 tooltip을 반복해서 숨기는 것이 직책 버튼과 능력 버튼 tooltip 유실 원인이다.
- 기본 행 바인딩과 선택 버튼 바인딩을 분리하고, 선택 영역의 실제 표시 상태가 바뀔 때만 `SetActive`를 호출한다. 선택 항목이 없는 행도 빈 목록으로 명시적으로 바인딩하여 이전 상태를 정리한다.
- pointer가 머문 상태에서 tooltip 문자열이 갱신되면 마지막 pointer 위치에 새 문자열을 즉시 다시 표시한다. 직책 버튼은 해당 인물·직책 효과와 교체 대상, 능력 버튼은 능력 효과·지속시간·남은 횟수를 표시한다.

### 21.2 위인 패널 내부 탭과 필터

- 위인 도메인 패널 상단에 `직책 슬롯`과 `위인 영입` 2개 탭을 둔다. 직책 탭에는 4개 직책 카드만, 영입 탭에는 인물 카드만 표시하여 서로 섞이지 않게 한다.
- 영입 탭에는 `영입 불가능`, `영입 가능`, `영입됨` 3개 독립 toggle을 둔다. 세 toggle은 기본 ON이며 조합에 따라 목록을 즉시 필터링한다.
- `영입 불가능`은 현재 후보도 활성 위인도 아닌 미발견·은퇴 인물, `영입 가능`은 현재 후보, `영입됨`은 영입 후 미배치 또는 직책 배치된 인물을 뜻한다.
- 탭·필터 UI는 `CivicDomainPanelView`와 `UiPrefabGenerator`의 고정 prefab pool로 생성하며 런타임 UI 계층을 만들지 않는다.

### 21.3 국가 통합 Modifier 패널

- 좌측 sidebar에 `국가` 버튼을 추가하고 기존 자원·건물·기술 패널과 같은 상호 배타적 detail panel로 연다.
- 국가 패널 상단에는 현재 설립 국가명을 표시한다. 국가 설립 모듈이 OFF이거나 아직 설립되지 않았으면 해당 상태를 명시하되, 다른 활성 시스템의 modifier 목록은 계속 표시한다.
- 목록은 활성 `CivicModifierLedger` 항목을 `effectType + targetId` 기준으로 합산하고, 연구 완료 기술의 수치 효과도 같은 화면에 포함한다. `plannedFollowUp`처럼 런타임 수치가 없는 효과는 제외한다.
- 각 통합 행은 효과명·대상·총 증감치를 표시한다. 우측 `+/-` 버튼으로 세부 영역을 열고 닫으며, 세부 영역에는 기술·국가·정치체계·위인·위인 능력·불가사의·사건·도전과제·환생 유산 등 source type과 source 명칭별 기여값을 표시한다.
- fixed row pool과 `LayoutElement` 높이 전환을 사용하여 펼침 시에만 행이 커지고, content가 panel보다 크면 기존 ScrollRect로 스크롤한다.

### 21.4 검증 기준

- 위인 직책 선택 버튼과 직책 카드의 능력 버튼을 hover하면 비어 있지 않은 tooltip이 표시된다.
- 매 프레임 재바인딩 중에도 tooltip이 사라지지 않는다.
- 위인 탭 전환 시 직책 카드와 인물 카드가 섞이지 않으며 세 필터의 모든 단독·조합 상태가 정확히 적용된다.
- 국가 버튼이 다른 좌측 패널을 닫고 국가 패널을 열며, 닫기·ESC·다른 sidebar 버튼 전환 규칙을 따른다.
- 국가 modifier 합계가 source 세부값과 일치하고, 연구 기술 효과와 활성 확장 모듈 효과가 식별 가능한 이름으로 표시된다.
- `GenerateUI`, `ValidateData`, `ValidateUI`, EditMode, PlayMode, MSBuild를 다시 통과한다.

### 21.5 구현·검증 결과 (2026-07-01)

- 직책 선택 버튼 4개와 직책 배치 후 능력 발동 버튼에 현재 인물·직책별 효과를 설명하는 tooltip을 연결했다.
- 위인 패널을 `직책 슬롯`/`위인 영입` 탭으로 분리하고 영입 불가능·영입 가능·영입됨 필터를 독립 toggle로 구현했다.
- 공용 tooltip에는 owner 기반 표시 수명주기를 추가했다. 숨겨진 다른 행의 `OnDisable()`은 자신이 열지 않은 tooltip을 닫지 못하며, 패널 닫기·전환은 기존 강제 `Hide()`로 모든 tooltip을 정리한다.
- 좌측 `국가` 버튼과 국가 detail panel을 추가했다. 활성 ledger와 연구 기술 수치 효과를 효과 종류·대상별로 합산하고, `+/-` 버튼으로 source별 기여 내역을 펼쳐 확인할 수 있다.
- 생성기 회귀 테스트에서 새 위인 탭 영역 수를 기존 1개로 단언한 항목을 실제 사양인 2개로 교정했다. PlayMode tooltip 테스트는 OS cursor가 발생시키는 정상 `PointerExit`와 매 프레임 `Bind()` 회귀를 분리하여 직접 rebind를 검증한다.
- 검증 통과: `Compile`, `GenerateUI`, `ValidateData`, `ValidateUI`, EditMode 68/68, PlayMode 10/10, `dotnet msbuild Civic/Civic.sln /p:RestorePackages=false /p:RestoreIgnoreFailedSources=true`.

## §22 국가 Modifier 그룹·HUD 성능·위인 능력 설명 보정 (2026-07-02)

### 22.1 국가 Modifier 그룹 표시

- 기존 국가 패널의 정본 leaf는 `effectType + targetId`별 합계와 source별 기여 내역이다. 서로 다른 단위나 적용 대상을 합산하면 실제 게임 효과와 다른 숫자가 되므로 leaf 계산은 유지한다.
- 화면 최상위 행은 같은 `effectType`을 하나의 표시 그룹으로 묶는다. 대상이 하나면 기존처럼 합계를 표시하고, 대상이 여러 개면 `N개 대상 · M개 출처`로 표시한다.
- 펼침 영역은 대상별 합계 아래 source별 기여를 들여쓰기해 표시한다. 기술 추가 산출은 건물·산출 자원별 leaf가 같은 그룹 아래 정렬되므로 건물별 증가 원인을 한 곳에서 확인할 수 있다. 건설 비용 multiplier와 고정값은 단위가 다르므로 별도 그룹을 유지한다.

### 22.2 `CivicHudController.Update()` 성능 원인과 수정 경계

- 관찰 현상은 Editor Profiler 기준 `CivicHudController.Update()` 100~350ms와 약 5 FPS다.
- 코드상 중복 원인은 세 가지다.
  1. `CivicGameSimulation.Advance()`가 1초 whole tick이 없는 프레임에도 `CalculateSnapshot()`을 호출한다.
  2. `CivicWonderModule.AfterAdvance()`가 같은 프레임에 다시 `RefreshSnapshot()`을 호출한다.
  3. controller가 매 프레임 `CivicHudView.Render()`와 `CivicModulePanelView.Bind()`를 호출하며, 닫힌 패널의 고정 row pool까지 문자열·active state·layout을 다시 바인딩한다.
- `ProfilerMarker`를 controller advance/core HUD/module HUD/event UI와 simulation snapshot 계산 경계에 추가해 baseline과 수정 후 값을 같은 PlayMode 시나리오에서 비교한다.
- core simulation은 1초 tick이 없으면 기존 snapshot을 재사용한다. 불가사의의 지속 소모·건설 진행은 상태 계산을 유지하되 전체 snapshot 갱신은 고정 UI refresh 경계에서 한 번만 수행한다.
- core HUD는 snapshot 참조나 사용자가 선택한 panel 상태가 바뀔 때만 다시 그리고, 모듈 HUD는 열려 있을 때만 제한된 주기로 갱신한다. 버튼 클릭·탭 전환·디버그 액션은 즉시 강제 갱신하여 조작 응답성을 유지한다.

### 22.3 위인 직책의 액티브 능력 표시

- 직책 배치 카드 설명란에 지속 직책 효과와 액티브 능력을 별도 절로 모두 표시한다.
- 활성화된 `능력 발동` 버튼 tooltip에는 현재 배치 인물, 능력명, 효과, 지속시간, 런당 사용 횟수와 남은 횟수를 표시한다.
- 비활성화 상태에도 능력이 없거나 남은 횟수가 0인 사유를 tooltip으로 확인할 수 있게 한다.

### 22.4 검증 기준

- 국가 패널에서 같은 효과 종류가 단일 상위 행으로 묶이고, 펼침 시 모든 대상별 합계와 source 기여가 누락 없이 표시된다.
- modifier 그룹화 전후 leaf 총액과 cap 적용 결과가 동일하다.
- 위인 직책 설명과 능력 버튼 tooltip에 동일한 액티브 능력 정의가 표시된다.
- 성능 계측은 닫힌 모듈 패널과 열린 위인 패널을 각각 측정하고, snapshot·core HUD·module HUD 비용을 로그로 남긴다.
- `Compile`, `GenerateUI`, `ValidateData`, `ValidateUI`, EditMode, PlayMode, MSBuild를 통과한다.

### 22.5 구현·계측 결과 (2026-07-02)

- 국가 modifier 화면은 `effectType`별 상위 그룹과 `effectType + targetId`별 leaf로 분리했다. 단일 대상 그룹은 합계를 바로 표시하고, 복수 대상 그룹은 대상 수·출처 수를 표시한 뒤 펼침 영역에서 대상별 합계와 source별 기여를 보여준다.
- `CivicGameSimulation.Advance()`는 1초 whole tick이 없는 프레임에서 기존 snapshot을 재사용한다. `CivicWonderModule.AfterAdvance()`의 매 프레임 중복 `RefreshSnapshot()`도 제거했다.
- HUD core는 snapshot 참조가 변경되거나 사용자 액션이 발생했을 때만 갱신한다. 모듈·이벤트 UI는 0.2초 주기로 갱신하고, 닫힌 모듈 패널은 재바인딩하지 않는다. 버튼·탭·디버그 액션은 기존처럼 즉시 강제 갱신한다.
- 건물별 GDP 예상치는 건물마다 전체 `CalculateRates()`를 재실행하지 않고 현재 가격·효율과 적용 후 산출 delta로 계산한다. 실제 GDP 정산식은 변경하지 않았다.
- 연구 완료 개수가 변할 때만 활성 기술 효과와 인구층 소비 자원 캐시를 다시 만들며, 기술 효과는 건물 ID별 인덱스로 조회한다.
- 기술 연구는 성능 비용과 직접 관련이 있었다. 시작 상태는 연구 3개·표시 자원 7개·건물 5개에서 snapshot 약 3.1~3.6ms였고, 전 기술 연구 상태는 연구 112개·표시 자원 31개·건물 29개로 증가했다.
- 전 기술 연구·모든 건물 100개 스트레스 snapshot은 최적화 전 196.25ms에서 최적화 후 22.46~25.03ms로 약 87~89% 감소했다. snapshot이 없는 평상시 `CivicHudController.Update()`는 닫힌 패널과 열린 패널 모두 약 0.003ms였고, 모듈 HUD refresh는 약 0.014~0.018ms였다.
- 위인 능력 CSV에 `displayNameKo`, `descriptionKo`를 추가했다. 직책 카드 설명과 능력 버튼 tooltip은 같은 정의에서 능력명·설명·실제 runtime 효과·지속시간·런당 횟수·남은 횟수를 표시한다.
- 검증 통과: `Compile`, `GenerateUI`, `ValidateData`, `ValidateUI`, EditMode 68/68, PlayMode 11/11, MSBuild.
- 잔여 특성: 전 기술 연구 상태에서는 1초 tick 시 약 25ms snapshot spike가 남지만, 기존처럼 매 프레임 100~350ms를 점유하지 않으므로 지속적인 5 FPS 원인은 제거됐다. 추가 최적화가 필요하면 resource flow/build projection을 snapshot core에서 lazy detail 계산으로 분리한다.

## §23 Tooltip Raycast·국가 Modifier 상세 높이 보정 (2026-07-02)

- Tooltip은 표시 전용 overlay이며 pointer 입력 대상이 아니다. Tooltip 배경·문자 `Graphic.raycastTarget`을 모두 끄고 `CanvasGroup.blocksRaycasts=false`, `interactable=false`로 생성한다. 따라서 커서 아래 Tooltip이 나타나도 원래 hover 대상의 `PointerExit`을 유발하지 않는다.
- 국가 modifier 목록의 `VerticalLayoutGroup`은 자식 높이를 `LayoutElement.preferredHeight`로 제어한다. 상세 Text는 `VerticalWrapMode.Overflow`를 사용하고 실제 `preferredHeight`에 padding을 더해 행 높이를 계산한다.
- 펼침/접힘 후 content RectTransform을 한 번 다시 layout하여 ScrollRect 전체 높이가 즉시 갱신되게 한다.
- EditMode는 Tooltip이 raycast를 차단하지 않는지와 국가 행 layout 설정을 검증한다. PlayMode는 hover 후 한 프레임 이상 Tooltip 유지와 여러 줄 modifier 상세 행의 실제 높이 증가를 검증한다.
